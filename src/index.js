...
async function shouldCreateNewLead(phone) {
  ...
  // Если карточка в финальном статусе или удалена/архивирована
  ...
  // Иначе — активная карточка, просто добавляем сообщение
...
async function handleBotStep(phone, chatId, messageText, messageId, waName) {
  ...
  if (session && session.step === 'done') {
    ...
    if (!create && existingLead) {
      // Активная карточка — просто сохраняем сообщение
      await saveMessage(existingLead.id, text, 'in', messageId);
      await supabase.rpc('increment_unread', { lead_id: existingLead.id });
      console.log(`💬 Сообщение добавлено в карточку ${existingLead.id}`);
      return;
    }

    // Нужна новая заявка — сбрасываем сессию и запускаем бота
...
app.get('/api/stats', async (req, res) => {
  ...
  res.json({ total:inPeriod.length, success:successAll.length, fail:inPeriod.filter(l=>l.status==='fail').length, new:inPeriod.filter(l=>l.status==='new').length, inProgress:all.filter(l=>l.status==='in_progress').length, waiting:all.filter(l=>l.status==='waiting').length, totalAmount, avgCheck:successAll.length>0?Math.round(totalAmount/successAll.length):0, conversion:inPeriod.length>0?Math.round(successAll.length/inPeriod.length*100):0, byCity, byDay, failReasons, overdue });
});
...
app.post('/cron/morning-report', async (req, res) => {
  ...
  msg += `📊 *Вчера:*\n`;
  msg += `✅ Успешно: *${ydSucc.length}* сделок на *${fmt(ydAmount)} ₸*\n`;
  msg += `❌ Провал: *${ydFail.length}*\n\n`;
...
app.post('/cron/check-new', async (req, res) => {
  ...
  newLeads.slice(0,10).forEach(l=>{msg+=`• ${l.client_name} — ${l.device} — ${l.city} — _${fmt(Date.now()-new Date(l.created_at).getTime())}_\n`;});
...
app.post('/cron/check-overdue', async (req, res) => {
  ...
  overdue.slice(0,10).forEach(l=>{msg+=`• ${l.client_name} — ${l.device} — ${l.city} — _${fmt(Date.now()-new Date(l.updated_at||l.created_at).getTime())}_\n`;});
...
app.post('/cron/check-waiting', async (req, res) => {
  ...
  overdue.slice(0,10).forEach(l=>{const vd=new Date(l.visit_date).toLocaleDateString('ru-RU');const od=fmt(now-new Date(l.visit_date).getTime());msg+=`• ${l.client_name} — ${l.device} — ${l.city}\n  Ожидали: ${vd}, просрочка _${od}_\n`;});
